...
const NavBar = () => {
  ...
  return (
    <div className={`w-screen py-5 px-[6%] flex justify-between fixed z-10 text-sm ${navdark ? "bg-black/90 backdrop-blur-sm" : "npm install firebasebg-[linear-gradient(180deg,rgba(0,0,0,0.8),transparent)]"} `}>
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="flex items-center relative group gap-1.5">
          ...
          <div className="absolute top-full right-0 w-max px-4 py-5 bg-gray-900 text-sm text-gray-400 hover:text-white rounded-xl z-1 cursor-pointer hidden group-hover:block">
            <p className="cursor-pointer underline" onClick={()=>logout()}>Sign Out of Netflix</p>
          </div>
        </div>
      </div>
    </div>
